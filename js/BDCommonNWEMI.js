...
var resPath = '../../pgmerc/bdqp/resources/';
...
$(document).ready(function() {
    ...
    if (MsgSrc.substring(0, 3) == "ETD" || MsgSrc.substring(0, 4) == "MIDC" || MsgSrc.split("|")[0].substring(MsgSrc.split("|")[0].length - 4) == "MWBU") {
        var challanbURL = resPath + '../../merchantui/scripts/echallanContext.js';
        var $head = $("head");
        $head.append($("<script/>", {
            src: challanbURL
        }));
...
$(document).on('click', '.emiSection table.table tr, input[name=emiPlan]', function() {
    if ($('#txtBankIDEMI').val().match(emiBankID)) {
        ...
        $('input[name=txtItemCode]').val(prodIDPrefix === "AE2" ? 'AM' + $(this).find('input[name=emiPlan]').attr('id') : prodIDPrefix === "ONECARDEMI" ? 'CRD_' + $(this).find('input[name=emiPlan]').attr('id') : prodIDPrefix + $(this).find('input[name=emiPlan]').attr('id'));
    }
});
...
TEZQUERY.define = {
    ...
    requestPGTransactionStatus: function($bankres, $tranId, $msgFlag) {
        try {
            ...
            if (document.form1.txtBankID.value == "CPH") {
                requestPGTransactionStatusHandler = "https://payments.billdesk.com/bankservices/UPIDirect";
                stopWatchTimer = 310;
            }
            if (!(document.form1.txtBankID.value == "CPI" || document.form1.txtBankID.value == "CPH") && document.form1.paymentMode.value == "UPI") {
